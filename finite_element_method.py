...
def solve_finite_element_method(params):
    ...
    Nt = min(15000, max(1500, int(tf / 100)))
    dt = tf / Nt
    ...
    for n in range(Nt):
        u_current = u.copy()
        dudt = np.zeros(Nr)
        u_avg_plus = 0.5 * (u_current[2:] + u_current[1:-1])
        u_avg_minus = 0.5 * (u_current[1:-1] + u_current[:-2])
        if p_nonlin == 0:
            D_eff_plus, D_eff_minus = D_param, D_param
        else:
            D_eff_plus = D_param * np.maximum(eps, 1 - u_avg_plus / u0) ** p_nonlin
            D_eff_minus = D_param * np.maximum(eps, 1 - u_avg_minus / u0) ** p_nonlin
        flux_plus = D_eff_plus * (u_current[2:] - u_current[1:-1]) * dr_inv
        flux_minus = D_eff_minus * (u_current[1:-1] - u_current[:-2]) * dr_inv
        diffusion_at_node = (flux_plus - flux_minus) * dr_inv
        S_plus = sc * u_avg_plus * (1 - u_avg_plus / u0)
        S_minus = sc * u_avg_minus * (1 - u_avg_minus / u0)
        source_at_node = 0.5 * (S_plus + S_minus)
        dudt[1:-1] = diffusion_at_node + source_at_node
        u_avg_boundary = 0.5 * (u_current[-1] + u_current[-2])
        if p_nonlin == 0:
            D_eff_boundary = D_param
        else:
            D_eff_boundary = D_param * max(eps, 1 - u_avg_boundary / u0) ** p_nonlin
        flux_boundary = D_eff_boundary * (u_current[-1] - u_current[-2]) * dr_inv
        diffusion_boundary = -flux_boundary * dr_inv
        source_boundary = sc * u_avg_boundary * (1 - u_avg_boundary / u0)
